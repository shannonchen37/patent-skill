...
def test_amendment_requires_original_basis() -> None:
    assert validate_amendment_basis([{"potential_amendment": "add F4", "specification_paragraph": None}])


def test_builder_preserves_proposed_enablement_gate() -> None:
    result = build_engineering_provenance([{
        "feature_id": "F5", "invention_id": "P001", "feature": "future option",
        "status": "proposed-but-enabled", "evidence": [], "inventor_confirmation": True,
